...
class Command(BaseCommand):
    ...
    def handle(self, provider, **options):
        ...
        if provider=='fireworks':
            args=('Fireworks',settings.FIREWORKS_API_KEY,settings.FIREWORKS_MODEL,'https://api.fireworks.ai/inference/v1/chat/completions')
            extra=settings.FIREWORKS_EXTRA
        ...
        try:
            if provider == 'auto':
                ...
                result, model=request(*args,[{'role':'user','content':content}],extra=extra)
