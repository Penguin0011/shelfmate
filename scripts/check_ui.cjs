...
await page.goto(baseURL);await page.getByRole('button',{name:'+ New box',exact:true}).click();await page.getByLabel('Box number',{exact:true}).fill('21');await page.getByLabel('Category / name').fill('Reusable test');await page.getByRole('button',{name:'Save box',exact:true}).click();await page.waitForURL('**/box/21');
...
await archiveBox();await page.goto(baseURL);await page.locator('summary').filter({hasText:'Archived boxes'}).click();await page.locator('.archived-boxes a').filter({hasText:'Reusable test'}).click();await page.getByRole('button',{name:'Restore / reuse Box 21',exact:true}).click();await page.getByLabel('Category / name').fill('Restored test');await page.getByRole('button',{name:'Restore box',exact:true}).click();await page.getByRole('status').filter({hasText:'Box restored'}).waitFor();
await archiveBox();await page.goto(baseURL);await page.getByRole('button',{name:'+ New box',exact:true}).click();await page.getByLabel('Box number',{exact:true}).fill('21');await page.getByLabel('Category / name').fill('Reassigned test');await page.getByRole('button',{name:'Save box',exact:true}).click();await page.waitForURL('**/box/21');await page.getByRole('status').filter({hasText:'Box restored'}).waitFor();assert.equal((await (await page.request.get('/api/boxes/21/')).json()).category,'Reassigned test');
assert.deepEqual(errors,[]);console.log('UI flows passed: search, household flag, login, manual entry/move, photo review/autosave/save, flag resolution, archive/restore and number reuse.');} finally {if(browser)await browser.close();server.kill();await new Promise(resolve=>{if(server.exitCode!==null)resolve();else server.once('exit',resolve);});fs.rmSync(directory,{recursive:true,force:true});}
})();
