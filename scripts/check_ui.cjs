...
await archiveBox();await page.goto(baseURL);await page.getByRole('button',{name:'+ New box',exact:true}).click();await page.getByLabel('Box number',{exact:true}).fill('21');await page.getByLabel('Category / name').fill('Reassigned test');assert.deepEqual(await page.locator('#room-choices option').evaluateAll(options=>options.map(o=>o.value)),[]);await page.getByLabel('Location (optional)').fill('Closet');await page.getByRole('button',{name:'Save box',exact:true}).click();await page.waitForURL('**/box/21');await page.getByRole('status').filter({hasText:'Box restored'}).waitFor();assert.equal((await (await page.request.get('/api/boxes/21/')).json()).category,'Reassigned test');
