...
await page.goto(baseURL);await page.getByRole('button',{name:'+ New',exact:true}).click();await page.getByLabel('Box number',{exact:true}).fill('21');await page.getByLabel('Category / name').fill('Reusable test');await page.getByLabel('Location (optional)').fill('Office');await page.getByRole('button',{name:'Save box',exact:true}).click();await page.waitForURL('**/box/21');
...
await archiveBox();await page.goto(baseURL);await page.getByRole('button',{name:'+ New',exact:true}).click();await page.getByLabel('Box number',{exact:true}).fill('21');await page.getByLabel('Category / name').fill('Reassigned test');assert.deepEqual(await page.locator('#room-choices option').evaluateAll(options=>options.map(o=>o.value)),[]);await page.getByLabel('Location (optional)').fill('Closet');await page.getByRole('button',{name:'Save box',exact:true}).click();await page.waitForURL('**/box/21');await page.getByRole('status').filter({hasText:'Box restored'}).waitFor();await page.getByRole('heading',{name:/Reassigned test/}).waitFor();assert.match(await page.locator('.box-hero>p').textContent(),/Closet/);
